...
use MXRVX\CodeStyle\Rules\DefaultRules;
...
class Builder
{
    ...
    public function include(string $path): self
    {
        ...
        \is_dir(
            $path,
        ) and $this->includeDirs[] = $path;
        \is_file(
            $path,
        ) and $this->includeFiles[] = $path;
        return $this;
    }
    ...
    public function exclude(string $path): self
    {
        ...
        \is_dir(
            $realPath,
        ) and $this->excludeDirs[] = $realPath;
        \is_file(
            $realPath,
        ) and $this->excludeFiles[] = $realPath;
        return $this;
    }
    ...
    public function build(): Config
    {
        ...
        $this->excludeDirs !== [] || $this->excludeFiles !== [] and $finder->filter(
            function (
                ...
                $initDir = $file->isDir() ? $file->getRealPath() : \dirname(
                    $file->getRealPath(),
                );
        ...
        $config->setRiskyAllowed(
            $this->allowRisky,
        );
        $this->cacheFile === null or $config->setCacheFile(
            $this->cacheFile,
        );
